import React from "react";
import { Container, Row, Col, Stack, Image } from "react-bootstrap";
import logoImage from "../../../assets/images/logos/lgms-hero-section.png";
import "../../../styles/homepage.css";

const HeroSection = () => {
  return (
    <Container fluid style={{ backgroundColor: "#cdf2d0", padding: "4rem 0" }}>
      <Row className="align-items-center">
        <Col lg={{ span: 8 }}>
          <Stack direction="vertical">
            <div className="p-2 flex items-center">
              <svg className="icon" viewBox="0 0 512 512">
                <path d="M480 179.6C498.6 188.4 512 212.1 512 240C512 267.9 498.6 291.6 480 300.4V448C480 460.9 472.2 472.6 460.2 477.6C448.3 482.5 434.5 479.8 425.4 470.6L381.7 426.1C333.7 378.1 268.6 352 200.7 352H192V480C192 497.7 177.7 512 160 512H96C78.33 512 64 497.7 64 480V352C28.65 352 0 323.3 0 288V192C0 156.7 28.65 128 64 128H200.7C268.6 128 333.7 101 381.7 53.02L425.4 9.373C434.5 .2215 448.3-2.516 460.2 2.437C472.2 7.39 480 19.06 480 32V179.6zM200.7 192H192V288H200.7C280.5 288 357.2 317.8 416 371.3V108.7C357.2 162.2 280.5 192 200.7 192V192z"></path>
              </svg>
              <span className="website-updates-text">Website Updates</span>
            </div>
            <div className="p-2 chatbot-text">
              Got Questions? Ask Our{" "}
              <span className="chatbot-link">Chatbot!</span>
            </div>
            <div className="p-2 description-text">
              Instant access FAQs with our 24/7 digital assistant, providing
              quick, accurate answers through an intuitive chat interface
            </div>
          </Stack>
        </Col>
        <Col lg="auto">
          <div className="school-logo-text">
            <div>
              <span className="font-bold">L</span>earning
            </div>
            <div>
              <span className="font-bold">G</span>arden
            </div>
            <div>
              <span className="font-bold">M</span>ontessori
            </div>
            <div>
              <span className="font-bold">S</span>chool
            </div>
          </div>
        </Col>
        <Col>
          <Image src={logoImage} alt="School Logo LGMS" fluid />
        </Col>
      </Row>
    </Container>
  );
};

export default HeroSection;
